...
import { TYPES, cards, SKILL_ARRAY } from "../../consts/index";
...
export class CardComponent implements OnInit {
  ...
  onSubmit(event) {
    ...
    if (!this.final) {
      this.errorService.error("着地カード設定してな(´・ω・`)");
      return;
    }
  ...
  findCardForSkillsArm() {
    const remainingSkills = this.skills.filter(
      (ts) => !this.prioritySkills.some((ps) => ps.name === ts)
    );
    ...
    let cardIndexedArray = Object.keys(cardIndexed)
      .map((key) => {
        return {
    ...
    for (let i = 0; i < len; i++) {
      if (
        cardIndexedArray[i].skills.every((s) =>
          tmpSkills.some((ts) => ts === s)
        )
      ) {
        cardIndexedArray = cardIndexedArray.map((ci, index) => {
          ...
          return {
            card: ci.card,
            skills: ci.skills.filter(
              (s) => !cardIndexedArray[i].skills.some((cs) => cs === s)
            ),
          };
        });
        tmpSkills = tmpSkills.filter(
          (ts) => !cardIndexedArray[i].skills.some((s) => ts === s)
        );
      }
    }
  ...
  findPrioritySkills() {
    const maxValuedSkills = this.final.skills.filter(
      (skill) =>
        this.cardService.findMaxValue(skill.name, this.limit) === skill.value
    );
    this.prioritySkills = maxValuedSkills.filter((skill) =>
      this.skills.some((s) => s === skill.name)
    );
  }

  findMaterialCard() {
    this.materialB = this.materialA = this.cardList.find(
      (card) => card.rank === 0 && !this.cardService.isExist(card)
    );
  ...
  fillSkillPools() {
    ...
    this.skillPoolB = this.skills.filter(
      (skill) => !this.skillPoolA.some((s) => s === skill)
    );
  }
  ...
  calcSteps() {
    ...
    this.skillPoolA.forEach((spA) => {
      ...
      const material = this.cardIndexedArray.find((cia) =>
        cia.skills.some((s) => s === spA)
      );
